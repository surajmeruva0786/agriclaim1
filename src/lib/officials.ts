...
export async function ensureSeedOfficials() {
	...
	const seed: OfficialRecord[] = [
		{ username: 'verifier1', role: 'Verifier', displayName: 'Document Verifier' },
		{ username: 'field1', role: 'FieldOfficer', displayName: 'Field Officer' },
		{ username: 'revenue1', role: 'RevenueOfficer', displayName: 'Revenue Officer' },
		{ username: 'treasury1', role: 'TreasuryOfficer', displayName: 'Treasury Officer' },
	...
	await Promise.all(
		seed.map(async (rec) => {
			...
			if (!snap.exists) {
				await ref.set({ role: rec.role, displayName: rec.displayName || rec.username, createdAt: new Date() }, { merge: true });
			}
		})
	);
}
